...
class AudioProcessorDLL:
    ...
    def _load_dll(self):
        ...
        try:
            # Try to load the DLL
            dll_path = os.path.join(os.path.dirname(__file__), "sunoready_audio.dll")
            if os.path.exists(dll_path):
                self.dll = ctypes.CDLL(dll_path)
                self._setup_function_signatures()
                self.dll_available = True
                print("✅ High-performance DLL loaded successfully!")
            else:
                print("⚠️ DLL not found - falling back to Python implementation")
                self.dll_available = False
    ...
    def _setup_function_signatures(self):
        ...
        # Filter functions
        self.dll.apply_lowpass_filter.argtypes = [
            ctypes.POINTER(ctypes.c_double),
            ctypes.c_int,
            ctypes.c_double,
            ctypes.c_double
        ]
        self.dll.apply_lowpass_filter.restype = ctypes.c_int
        ...
        # RMS calculation
        self.dll.get_audio_rms.argtypes = [
            ctypes.POINTER(ctypes.c_double),
            ctypes.c_int
        ]
        self.dll.get_audio_rms.restype = ctypes.c_double
